...
<style>
  ...
  /* sidebar thấp hơn modal */
  .sidebar, .doan-sidebar { z-index: 1000 !important; }

  /* chatbox nên thấp hơn modal để không che modal */
  #aiEventChatWidget { z-index: 1500 !important; }
</style>
@endpush
        ...
        <tbody>
          @forelse($events as $i => $e)
            <tr>
              ...
              <td class="event-location">{{ $e->DiaDiem }}</td>
              <td>{{ $e->SoLuongToiDa ?? 'Không giới hạn' }}</td>
...
=========================== --}}
@foreach($events as $e)
<div class="modal fade" id="modalEdit{{ $e->MaSK }}" tabindex="-1" data-bs-backdrop="static" data-bs-keyboard="false">
  <div class="modal-dialog modal-lg modal-dialog-centered">
    <div class="modal-content">
      ...
      <form method="POST" action="{{ route('doan.sukien.update') }}" enctype="multipart/form-data">
        @csrf
        <input type="hidden" name="MaSK" value="{{ $e->MaSK }}">
        ...
        <div class="modal-body">
          ...
          <div class="row">
            <div class="col-md-6 mb-2">
              ...
              <input type="datetime-local" class="form-control" name="ThoiGianBatDau"
                     value="{{ \Carbon\Carbon::parse($e->ThoiGianBatDau)->format('Y-m-d\TH:i') }}" required>
            </div>
            <div class="col-md-6 mb-2">
              <label class="form-label">Thời gian kết thúc</label>
              <input type="datetime-local" class="form-control" name="ThoiGianKetThuc"
                     value="{{ \Carbon\Carbon::parse($e->ThoiGianKetThuc)->format('Y-m-d\TH:i') }}" required>
            </div>
          </div>
          ...
          <div class="row">
            ...
            <div class="col-md-6 mb-2">
              <label class="form-label">Số lượng tối đa</label>
              <input type="number" class="form-control" name="SoLuongToiDa" min="1" value="{{ $e->SoLuongToiDa }}">
              <small class="text-muted">Để trống = không giới hạn</small>
            </div>
          </div>
